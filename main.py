...
EQUIP = {
    ...
    "سپر پلاسما":(2400,"🛡️","فناوری",55),"زره انرژی":(2200,"🛡️","فناوری",48),
    "ماهواره شناسایی":(2800,"🛰️","فناوری",65),"پهپاد شناسایی":(1900,"🔭","فناوری",38),
    "ناو هواپیمابر":(5200,"🚢","دریایی",180),"مرکز فرماندهی":(4500,"🏰","راهبردی",150),
}
...
def transfer_coins(data, sender, target, amount):
    sender=str(sender); target=str(target)
    if sender==target: return False,"نمی‌توانید به خودتان انتقال دهید."
    if target not in data.get("users",{}): return False,"شناسه مقصد پیدا نشد."
    if sender not in data.get("users",{}): return False,"کاربر فرستنده پیدا نشد."
    try: amount=int(amount)
    except: return False,"مقدار نامعتبر است."
    if amount<=0: return False,"مقدار باید مثبت باشد."
    u=data["users"][sender]
    today=date.today().isoformat()
    if u.get("transfer_date")!=today:
        u["transfer_date"]=today; u["transfer_today"]=0
    used=int(u.get("transfer_today",0))
    if used+amount>DAILY_TRANSFER_LIMIT:
        return False,f"سقف انتقال روزانه ۵,۰۰۰ سکه است. باقی‌مانده امروز: {fn(max(0,DAILY_TRANSFER_LIMIT-used))}"
    balance=int(u.get("coins",0))
    if balance<amount: return False,f"موجودی کافی نیست. موجودی: {fn(balance)}"
    data["users"][sender]["coins"]=balance-amount
    data["users"][target]["coins"]=int(data["users"][target].get("coins",0))+amount
    u["transfer_today"]=used+amount
    tx={"id":f"TX-{int(time.time()*1000)}-{random.randint(100,999)}","from":sender,"to":target,"amount":amount,"time":datetime.now().isoformat()}
    data.setdefault("coin_transfers",[]).append(tx)
    data["coin_transfers"]=data["coin_transfers"][-2000:]
    save_data(data)
    return True,tx
...
def load_data():
    ...
    for k,v in {"users":{},"banned_users":[],"user_eq":{},"user_packs":{},"attack_logs":[],"bot_country":None,"bot_last_action":None,"daily_rewards":{},"coin_transfers":[],"country_claimed":{},"faction_claims":{},"damage_resets":[]}.items():
        if k not in d: d[k]=v
    for uid,u in d.get("users",{}).items():
        u.setdefault("warnings",0); u.setdefault("warning_log",[]); u.setdefault("daily_statements",{}); u.setdefault("faction",None); u.setdefault("country_claimed",False); u.setdefault("faction_claimed",[]); u.setdefault("country_left_at",None); u.setdefault("transfer_today",0); u.setdefault("transfer_date",date.today().isoformat())
    return d
def save_data(data):
    try:
...
async def leave_country(bot,cid,data,countries,uid):
    ...
    data["users"][uid]["country_left_at"]=datetime.now().isoformat()
    data["user_eq"][uid]={}
    data["user_packs"][uid]=[]
...
def get_main_menu():
    ...
    b.row(b.button(id="alliance_menu",text="🤝 اتحادها"),b.button(id="transfer_coins",text="🪙 انتقال سکه"))
    b.row(b.button(id="battle_report",text="📜 گزارش جنگ"),b.button(id="rules",text="📜 قوانین"))
    b.row(b.button(id="my_warnings",text="⚠️ اخطارهای من"),b.button(id="leave_country",text="🚪 خروج از کشور"))
    return b.build(resize_keyboard=True,on_time_keyboard=True)
...
def get_admin_countries_kb(countries,page=0):
    b=ChatKeypadBuilder(); items=list(countries.items()); start=page*12; end=start+12
    row=[]
    for code,info in items[start:end]:
        owner="🤖" if info.get("owner")=="BOT_AI" else ("🟢" if info.get("owner") else "⚪")
        row.append(b.button(id=f"ad_country_view_{code}",text=f"{owner} {info['flag']} {info['name'][:8]}"))
        if len(row)==3: b.row(*row); row=[]
    if row: b.row(*row)
    nav=[]
    if page>0: nav.append(b.button(id=f"ad_countries_page_{page-1}",text="◀️ قبل"))
    nav.append(b.button(id="ad_back_to_admin",text="🏠 پنل"))
    if end<len(items): nav.append(b.button(id=f"ad_countries_page_{page+1}",text="بعد ▶️"))
    b.row(*nav)
    return b.build(resize_keyboard=True,on_time_keyboard=True)

def admin_countries_text(data,countries,page=0):
    items=list(countries.items()); start=page*12; end=start+12
    lines=[f"🌍 مدیریت کشورها | صفحه {page+1}/{max(1,(len(items)+11)//12)}",""]
    for code,info in items[start:end]:
        if info.get("owner"):
            own=info["owner"]; name="🤖 ربات" if own=="BOT_AI" else data["users"].get(own,{}).get("username",own[:12])
            lines.append(f"{info['flag']} {info['name']} | 👤 {name} | 🆔 {own} | 💥 {fn(info.get('damage_taken',0))}")
        else: lines.append(f"{info['flag']} {info['name']} | ⚪ آزاد")
    return "\n".join(lines)
...
def get_admin_panel(bs):
    ...
    b.row(b.button(id="ad_reset_damage",text="💥 ریست خسارت‌ها"),b.button(id="ad_reset_user",text="👑 ریست کاربر"))
    ...
    b.row(b.button(id="ad_give_country",text="🌍 اعطای کشور"),b.button(id="ad_reset_transfer",text="🪙 ریست انتقال روزانه"))
    ...
    try:
        ...
        if user_states.get(cid,{}).get("transfer_target"):
            ...
            if target not in data.get("users",{}):
                await bot_instance.send_message(cid,"❌ شناسه مقصد پیدا نشد.",chat_keypad=get_main_menu()); user_states[cid]={}; return
            if target==uid:
                await bot_instance.send_message(cid,"❌ نمی‌توانید به خودتان سکه بفرستید.",chat_keypad=get_main_menu()); user_states[cid]={}; return
            user_states[cid]={"transfer_amount":True,"target":target}
            await bot_instance.send_message(cid,f"🪙 مقدار انتقال به `{target}` را وارد کنید.\nحداکثر انتقال روزانه: ۵,۰۰۰ سکه",chat_keypad=get_cancel()); return
        if user_states.get(cid,{}).get("transfer_amount"):
            try: amount=int(text.strip())
            except: await bot_instance.send_message(cid,"❌ فقط عدد وارد کنید."); return
            target=user_states[cid]["target"]
            ok,res=transfer_coins(data,uid,target,amount)
            if not ok:
                await bot_instance.send_message(cid,f"❌ {res}",chat_keypad=get_main_menu())
            else:
                await bot_instance.send_message(cid,f"✅ انتقال موفق بود!\n🪙 مبلغ: {fn(amount)}\n👤 مقصد: `{target}`\n🧾 تراکنش: `{res['id']}`",chat_keypad=get_main_menu())
                try: await bot_instance.send_message(target,f"🪙 {gid(msg,uid)} برای شما {fn(amount)} سکه فرستاد.\n🧾 `{res['id']}`")
                except: pass
        ...
        if user_states.get(cid,{}).get("creating_alliance"):
            ...
            await bot_instance.send_message(cid,f"✅ اتحاد **{new_name}** ساخته شد! 👑",chat_keypad=get_alliance_menu(True,True))
            user_states[cid]={}; return
        ...
        if in_admin:
            ...
            if cb and cb.startswith("ad_country_view_"):
                code=cb.replace("ad_country_view_",""); info=countries.get(code)
                if not info: return await bot_instance.send_message(cid,"❌ کشور نامعتبر")
                own=info.get("owner"); own_name="🤖 ربات" if own=="BOT_AI" else (data["users"].get(own,{}).get("username",own[:12]) if own else "⚪ آزاد")
                await bot_instance.send_message(cid,f"{info['flag']} {info['name']}\n👤 مالک: {own_name}\n🆔 {own or '-'}\n💥 خسارت: {fn(info.get('damage_taken',0))}",chat_keypad=get_admin_back()); return
            if cb=="ad_countries":
                await bot_instance.send_message(cid,admin_countries_text(data,countries,0),chat_keypad=get_admin_countries_kb(countries,0)); return
            if cb and cb.startswith("ad_countries_page_"):
                page=max(0,int(cb.replace("ad_countries_page_","")))
                await bot_instance.send_message(cid,admin_countries_text(data,countries,page),chat_keypad=get_admin_countries_kb(countries,page)); return
            if cb=="ad_users":
                txt="👥 کاربران:\n"
            ...
            if cb=="ad_reset_damage":
                for info in countries.values(): info["damage_taken"]=0
                data.setdefault("damage_resets",[]).append({"admin":uid,"time":datetime.now().isoformat(),"scope":"all"})
                data["damage_resets"]=data["damage_resets"][-200:]
                save_data(data); save_countries(countries)
                await bot_instance.send_message(cid,"✅ خسارت تمام کشورها ریست شد.",chat_keypad=get_admin_panel(bs)); return
            if cb=="ad_reset_transfer":
                user_states[cid]={"reset_transfer_uid":True}
                await bot_instance.send_message(cid,"🪙 شناسه کاربر برای ریست سقف انتقال امروز:",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("reset_transfer_uid"):
                target=text.strip()
                if target not in data.get("users",{}): await bot_instance.send_message(cid,"❌ کاربر یافت نشد!")
                else:
                    data["users"][target]["transfer_date"]=date.today().isoformat(); data["users"][target]["transfer_today"]=0; save_data(data)
                    await bot_instance.send_message(cid,"✅ سقف انتقال روزانه کاربر ریست شد.")
                user_states[cid]={}; return
        ...
        if cb=="transfer_coins":
            user_states[cid]={"transfer_target":True}
            await bot_instance.send_message(cid,"👤 شناسه کاربر مقصد را وارد کنید:",chat_keypad=get_cancel()); return
        ...
        if cb and cb.startswith("join_alliance_"):
            ...
            try:
                if datetime.fromisoformat(adata.get("traitor_until",{}).get(uid,"1970-01-01T00:00:00")) > datetime.now():
                    return await bot_instance.send_message(cid,"⛔ هنوز زمان بازگشت شما تمام نشده است.")
            except Exception: pass
        ...
        if cb=="alliance_leave":
            ...
            if name in adata.get("alliances",{}):
                adata["alliances"][name]["members"]=[m for m in adata["alliances"][name].get("members",[]) if m!=uid]
            save_alliance(adata)
            await bot_instance.send_message(cid,f"🚪 از اتحاد **{name}** خارج شدید.",chat_keypad=get_main_menu()); return
        ...
        if cb and cb.startswith("country_"):
            ...
            first_country=not bool(data["users"][uid].get("country_claimed",False))
            if first_country: addc(data,uid,1000)
            countries[code]["owner"]=uid; data["users"][uid]["has_country"]=True; data["users"][uid]["country_claimed"]=True
            save_data(data); save_countries(countries)
            reward="\n🪙 پاداش شروع: ۱,۰۰۰ کوین" if first_country else "\nℹ️ پاداش شروع دوباره تعلق نمی‌گیرد."
            await bot_instance.send_message(cid,f"🎉 کشور {info['flag']} {info['name']} تصرف شد!{reward}",chat_keypad=get_main_menu()); return
        if cb=="buy_single":
        ...
        if cb and cb.startswith("faction_"):
            ...
            if faction_key in FACTIONS:
                ...
                claims=set(data["users"][uid].get("faction_claimed",[]))
                data["users"][uid]["faction"]=faction_key
                if faction_key not in claims:
                    for w in f["w"]: addeq(data,uid,w,50)
                    addc(data,uid,3000)
                    claims.add(faction_key)
                    data["users"][uid]["faction_claimed"]=list(claims)
                    reward="\n🎁 پاداش عضویت فقط یک‌بار برای هر گروهک"
                else:
                    reward="\nℹ️ پاداش عضویت قبلاً دریافت شده است."
                save_data(data)
                await bot_instance.send_message(cid,f"✅ به {f['name']} پیوستید!{reward}",chat_keypad=get_main_menu()); return
    except Exception as e:
