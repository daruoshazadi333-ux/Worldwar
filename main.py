...
FACTIONS = {
    ...
    "tech_union":{"name":"اتحاد فناوری","icon":"💻⚡","emoji":"🔷","atk":1.35,"def":1.75,"w":["مرکز سایبری","ماهواره شناسایی","رادار پیشرفته"],"min":4500,"max":15},
    "ocean_pact":{"name":"پیمان اقیانوس","icon":"🌊⚓","emoji":"🌊","atk":1.45,"def":1.55,"w":["ناوگان دریایی","زیردریایی","ناو"],"min":5500,"max":12},
    "alpha_guard":{"name":"گارد آلفا","icon":"🛡️⭐","emoji":"⭐","atk":1.25,"def":2.25,"w":["سپر انرژی","پدافند لیزری","مرکز فرماندهی"],"min":6000,"max":10},
    "trade_league":{"name":"اتحادیه تجاری","icon":"💰🤝","emoji":"🟡","atk":1.15,"def":1.2,"w":["بانک مرکزی","بازار جهانی","انبار لجستیک"],"min":3000,"max":20},
    "space_command":{"name":"فرماندهی فضایی","icon":"🛰️🌌","emoji":"🌌","atk":1.7,"def":1.4,"w":["ماهواره شناسایی","شبکه ماهواره‌ای","مرکز فرماندهی"],"min":7000,"max":8}
}
...
EQUIP.update({
    "سپر انرژی":(2200,"🛡️","دفاعی",75), "پدافند لیزری":(2600,"🔴","دفاعی",90),
    "رادار پیشرفته":(1800,"📡","اطلاعاتی",45), "ماهواره شناسایی":(3200,"🛰️","اطلاعاتی",110),
    "شبکه ماهواره‌ای":(4800,"🌌","اطلاعاتی",180), "مرکز سایبری":(3500,"💻","فناوری",130),
    "مرکز فرماندهی":(4200,"🏰","زیرساخت",150), "ناوگان دریایی":(5200,"⚓","دریایی",220),
    "کارخانه تجهیزات":(3900,"🏭","اقتصادی",120), "بانک مرکزی":(4500,"🏦","اقتصادی",100),
    "بازار جهانی":(3000,"📈","اقتصادی",80), "انبار لجستیک":(2800,"📦","لجستیک",70),
    "نیروگاه انرژی":(3400,"⚡","زیرساخت",95), "مرکز پزشکی":(2500,"🏥","پشتیبانی",60),
    "سامانه ارتباطی":(2400,"📶","فناوری",65), "هوش مصنوعی فرماندهی":(6500,"🤖","ویژه",250),
    "سپر پلاسما":(7000,"🔰","ویژه",300), "ناوگان رباتیک":(6000,"🤖","ویژه",240),
    "مرکز تحقیقات":(5000,"🧪","فناوری",170), "ابررادار":(5600,"📡","اطلاعاتی",200),
    "پایگاه لجستیک":(3600,"🚚","لجستیک",105)
})
...
def defense_power(data,uid):
    eq=data.get("user_eq",{}).get(uid,{})
    # تجهیزات دفاعی قدیمی + تجهیزات جدیدی که قابلیت دفاعی دارند.
    defensive_names=["پدافند","اس۴۰۰","پاتریوت","تاد","گنبد","اس۵۰۰",
                     "سپر انرژی","پدافند لیزری","مرکز سایبری","مرکز فرماندهی",
                     "نیروگاه انرژی","سپر پلاسما","سامانه ارتباطی","ناوگان رباتیک","انبار لجستیک"]
    dp=sum(EQUIP.get(it,(0,)*4)[3]*min(5,max(0,int(eq.get(it,0)))) for it in defensive_names)
    fac=data.get("users",{}).get(uid,{}).get("faction")
...
def load_un():
    if UN_FILE.exists():
        d=safe_json_load(UN_FILE,{})
        if d: return d
    d={"leader":None,"members":[],"requests":[],"resolutions":[],"created_at":datetime.now().isoformat()}
    save_un(d); return d
def save_un(d):
    try:
...
def load_data():
    ...
    for k,v in {"users":{},"banned_users":[],"user_eq":{},"user_packs":{},"attack_logs":[],"bot_country":None,"bot_last_action":None,"daily_rewards":{},"coin_transfers":[],"transfer_daily":{},"reward_codes":{},"reward_code_claims":{},"trades":[],"market":{},"economy_log":[]}.items():
        if k not in d: d[k]=v
    for uid,u in d.get("users",{}).items():
        u.setdefault("warnings",0); u.setdefault("warning_log",[]); u.setdefault("daily_statements",{}); u.setdefault("faction",None); u.setdefault("country_history",[]); u.setdefault("faction_history",[]); u.setdefault("reward_claims",[]); u.setdefault("trade_stats",{"bought":0,"sold":0})
    return d
def save_data(data):
    try:
...
def load_bot_status():
    if BOT_STATUS_FILE.exists():
        d=safe_json_load(BOT_STATUS_FILE,{})
        if d: return d
    d={"online":True}; save_bot_status(d); return d
def save_bot_status(s):
    try:
        with BOT_STATUS_FILE.open('w',encoding='utf-8') as f: json.dump(s,f,indent=4,ensure_ascii=False)
    except: pass

# ---------- Ultimate economy / rewards / trade ----------
MAX_DAILY_TRANSFER = 5000
MARKET_ITEMS = {
    "طلا":{"emoji":"🪙","base":500,"min":250,"max":900}, "نفت":{"emoji":"🛢️","base":700,"min":350,"max":1200},
    "غذا":{"emoji":"🌾","base":300,"min":150,"max":600}, "فناوری":{"emoji":"💻","base":1000,"min":500,"max":1800},
    "انرژی":{"emoji":"⚡","base":800,"min":400,"max":1500},
}
def ensure_extra_data(data):
    data.setdefault("transfer_daily",{}); data.setdefault("reward_codes",{}); data.setdefault("reward_code_claims",{}); data.setdefault("trades",[]); data.setdefault("market",{}); data.setdefault("economy_log",{}); data.setdefault("admin_accounts",{})
    if isinstance(data["economy_log"],dict): data["economy_log"]=[]
    for k,v in MARKET_ITEMS.items(): data["market"].setdefault(k,v["base"])
    return data
def daily_transfer_used(data,uid):
    ensure_extra_data(data); today=date.today().isoformat(); rec=data["transfer_daily"].get(uid,{})
    return int(rec.get("amount",0)) if rec.get("date")==today else 0
def transfer_coins_safe(data,sender,target,amount):
    ensure_extra_data(data)
    if sender==target:return False,"❌ انتقال به خودتان مجاز نیست."
    if target not in data.get("users",{}):return False,"❌ شناسه مقصد پیدا نشد."
    try:amount=int(amount)
    except:return False,"❌ مقدار نامعتبر است."
    if amount<=0:return False,"❌ مقدار باید بیشتر از صفر باشد."
    used=daily_transfer_used(data,sender)
    if used+amount>MAX_DAILY_TRANSFER:return False,f"❌ سقف انتقال روزانه ۵,۰۰۰ سکه است.\n📊 مصرف امروز: {fn(used)}\n📌 باقی‌مانده: {fn(MAX_DAILY_TRANSFER-used)}"
    if get_coins(data,sender)<amount:return False,f"❌ موجودی کافی نیست. موجودی: {fn(get_coins(data,sender))}"
    data["users"][sender]["coins"]-=amount; data["users"][target]["coins"]=get_coins(data,target)+amount
    today=date.today().isoformat(); rec=data["transfer_daily"].setdefault(sender,{"date":today,"amount":0})
    if rec.get("date")!=today:rec.update({"date":today,"amount":0})
    rec["amount"]+=amount; tid=f"TR-{int(time.time()*1000)}-{random.randint(100,999)}"
    data["coin_transfers"].append({"id":tid,"from":sender,"to":target,"amount":amount,"time":datetime.now().isoformat()}); data["coin_transfers"]=data["coin_transfers"][-1000:]
    save_data(data); return True,tid
def reset_transfer_limit(data,uid=None):
    ensure_extra_data(data)
    if uid and uid in data["transfer_daily"]:data["transfer_daily"].pop(uid,None)
    elif not uid:data["transfer_daily"]={}
    save_data(data)
def create_reward_code(data,code,coins=0,equipment=None,uses=1,expires_days=0):
    ensure_extra_data(data); code=str(code).strip().upper()
    if not code or code in data["reward_codes"]:return False,"❌ کد تکراری یا نامعتبر است."
    try:coins=int(coins);uses=int(uses);expires_days=int(expires_days)
    except:return False,"❌ مقدار نامعتبر است."
    if coins<0 or uses<1:return False,"❌ مقدار یا تعداد استفاده نامعتبر است."
    exp=(datetime.now()+timedelta(days=expires_days)).isoformat() if expires_days>0 else None
    data["reward_codes"][code]={"coins":coins,"equipment":equipment or {},"uses":uses,"used":0,"expires":exp,"created":datetime.now().isoformat()}; save_data(data); return True,code
def redeem_reward_code(data,uid,code):
    ensure_extra_data(data); code=str(code).strip().upper(); rec=data["reward_codes"].get(code)
    if not rec:return False,"❌ کد جایزه پیدا نشد."
    if rec.get("expires"):
        try:
            if datetime.now()>datetime.fromisoformat(rec["expires"]):return False,"⏰ این کد منقضی شده است."
        except:pass
    if int(rec.get("used",0))>=int(rec.get("uses",1)):return False,"❌ ظرفیت استفاده از کد تمام شده است."
    claims=data["reward_code_claims"].setdefault(code,[])
    if uid in claims:return False,"⚠️ شما قبلاً این کد را استفاده کرده‌اید."
    coins=int(rec.get("coins",0)); data["users"][uid]["coins"]=get_coins(data,uid)+coins
    for eq,amount in rec.get("equipment",{}).items():
        if eq in EQUIP and int(amount)>0:data.setdefault("user_eq",{}).setdefault(uid,{})[eq]=data["user_eq"].setdefault(uid,{}).get(eq,0)+int(amount)
    claims.append(uid); rec["used"]=int(rec.get("used",0))+1; data["users"][uid].setdefault("reward_claims",[]).append(code); save_data(data)
    return True,f"🎁 پاداش دریافت شد!\n🪙 +{fn(coins)} سکه\n🧰 تجهیزات: {', '.join(f'{k}×{v}' for k,v in rec.get('equipment',{}).items()) or 'ندارد'}"
def market_tick(data):
    ensure_extra_data(data); now=datetime.now(); last=data.get("market_tick")
    if last:
        try:
            if (now-datetime.fromisoformat(last)).total_seconds()<300:return
        except:pass
    for k,meta in MARKET_ITEMS.items():data["market"][k]=max(meta["min"],min(meta["max"],int(data["market"].get(k,meta["base"])*random.uniform(.92,1.08))))
    data["market_tick"]=now.isoformat(); save_data(data)
def trade_item(data,uid,item,qty,side):
    ensure_extra_data(data); market_tick(data)
    if item not in MARKET_ITEMS:return False,"❌ کالای نامعتبر."
    try:qty=int(qty)
    except:return False,"❌ تعداد نامعتبر است."
    if qty<=0 or qty>1000:return False,"❌ تعداد باید بین ۱ تا ۱۰۰۰ باشد."
    price=int(data["market"][item]); total=price*qty; inv=data["users"][uid].setdefault("trade_inventory",{})
    if side=="buy":
        if get_coins(data,uid)<total:return False,f"❌ کوین کافی نیست: {fn(total)}"
        data["users"][uid]["coins"]-=total; inv[item]=inv.get(item,0)+qty; data["users"][uid]["trade_stats"]["bought"]+=qty
    else:
        if inv.get(item,0)<qty:return False,"❌ موجودی کالا کافی نیست."
        inv[item]-=qty; data["users"][uid]["coins"]+=total; data["users"][uid]["trade_stats"]["sold"]+=qty
    data["trades"].append({"uid":uid,"item":item,"qty":qty,"side":side,"price":price,"total":total,"time":datetime.now().isoformat()}); data["trades"]=data["trades"][-1000:]; save_data(data)
    return True,f"✅ {'خرید' if side=='buy' else 'فروش'} انجام شد.\n{MARKET_ITEMS[item]['emoji']} {item} × {fn(qty)}\n💰 مبلغ: {fn(total)}\n🪙 موجودی: {fn(get_coins(data,uid))}"

def country_details(data,countries,code):
    i=countries.get(code)
    if not i:return "❌ کشور پیدا نشد."
    owner=i.get("owner"); owner_name="🤖 ربات" if owner=="BOT_AI" else (data.get("users",{}).get(owner,{}).get("username",owner or "آزاد") if owner else "آزاد")
    return f"🌍 {i['flag']} {i['name']}\n\n🆔 کد: {code}\n👤 رئیس: {owner_name}\n🔑 شناسه: `{owner or '-'}`\n🛡️ دفاع: {'فعال' if i.get('defense') else 'خاموش'}\n💥 خسارت: {fn(i.get('damage_taken',0))}\n⚡ پاداش قدرت: {fn(i.get('power_bonus',0))}"

def get_admin_country_detail_kb(code):
    b=ChatKeypadBuilder(); b.row(b.button(id=f"ad_country_owner_{code}",text="🔄 تغییر رئیس"),b.button(id=f"ad_country_reset_{code}",text="♻️ ریست")); b.row(b.button(id="ad_countries",text="🌍 بازگشت به کشورها"),b.button(id="ad_back_to_admin",text="👑 ادمین")); return b.build(resize_keyboard=True,on_time_keyboard=True)

def get_trade_menu():
    b=ChatKeypadBuilder(); b.row(b.button(id="trade_market",text="📈 بازار"),b.button(id="trade_buy",text="🛒 خرید")); b.row(b.button(id="trade_sell",text="💰 فروش"),b.button(id="trade_inventory",text="📦 انبار")); b.row(b.button(id="back_to_menu",text="🏠 بازگشت")); return b.build(resize_keyboard=True,on_time_keyboard=True)
...
async def broadcast_war(bot,data,countries,attacker_uid,target,eq_name,amt,damage,won,extra=""):
    ...
    for u in data["users"]:
        try: await bot.send_message(u,msg); await asyncio.sleep(0.01)
        except: pass
...
# ---------- تجهیزات ویژه ----------
# هر تجهیز علاوه بر قدرت پایه، یک قابلیت واقعی در نبرد دارد.
EQUIP_ABILITIES = {
    "سپر انرژی": {"dmg_reduce": 0.12, "desc": "🛡️ ۱۲٪ کاهش خسارت دریافتی"},
    "پدافند لیزری": {"def_mult": 1.18, "desc": "🔴 ۱۸٪ افزایش پدافند"},
    "رادار پیشرفته": {"win_bonus": 0.04, "desc": "📡 ۴٪ افزایش شانس برد"},
    "ماهواره شناسایی": {"win_bonus": 0.07, "desc": "🛰️ ۷٪ افزایش شانس برد"},
    "شبکه ماهواره‌ای": {"win_bonus": 0.10, "loot_bonus": 0.12, "desc": "🌌 ۱۰٪ شانس برد + ۱۲٪ شانس غنیمت"},
    "مرکز سایبری": {"attack_mult": 1.10, "def_mult": 1.06, "desc": "💻 ۱۰٪ افزایش قدرت حمله + ۶٪ پدافند"},
    "مرکز فرماندهی": {"attack_mult": 1.08, "def_mult": 1.10, "desc": "🏰 ۸٪ حمله + ۱۰٪ پدافند"},
    "ناوگان دریایی": {"attack_mult": 1.14, "desc": "⚓ ۱۴٪ افزایش قدرت حمله"},
    "کارخانه تجهیزات": {"attack_mult": 1.06, "loot_bonus": 0.10, "desc": "🏭 ۶٪ حمله + ۱۰٪ غنیمت"},
    "بانک مرکزی": {"coin_bonus": 50, "desc": "🏦 هنگام برد ۵۰ کوین اضافه"},
    "بازار جهانی": {"coin_bonus": 80, "desc": "📈 هنگام برد ۸۰ کوین اضافه"},
    "انبار لجستیک": {"dmg_reduce": 0.08, "desc": "📦 ۸٪ کاهش خسارت دریافتی"},
    "نیروگاه انرژی": {"attack_mult": 1.05, "def_mult": 1.05, "desc": "⚡ ۵٪ افزایش حمله و پدافند"},
    "مرکز پزشکی": {"repair": 350, "desc": "🏥 پس از نبرد ۳۵۰ خسارت کشور خودی را ترمیم می‌کند"},
    "سامانه ارتباطی": {"win_bonus": 0.06, "dmg_reduce": 0.04, "desc": "📶 ۶٪ شانس برد + ۴٪ کاهش خسارت"},
    "هوش مصنوعی فرماندهی": {"attack_mult": 1.25, "win_bonus": 0.05, "desc": "🤖 ۲۵٪ حمله + ۵٪ شانس برد"},
    "سپر پلاسما": {"dmg_reduce": 0.25, "def_mult": 1.08, "desc": "🔰 ۲۵٪ کاهش خسارت + ۸٪ پدافند"},
    "ناوگان رباتیک": {"attack_mult": 1.18, "dmg_reduce": 0.05, "desc": "🤖 ۱۸٪ حمله + ۵٪ کاهش خسارت"},
    "مرکز تحقیقات": {"attack_mult": 1.12, "win_bonus": 0.04, "desc": "🧪 ۱۲٪ حمله + ۴٪ شانس برد"},
    "ابررادار": {"win_bonus": 0.12, "desc": "📡 ۱۲٪ افزایش شانس برد"},
    "پایگاه لجستیک": {"attack_mult": 1.07, "loot_bonus": 0.18, "desc": "🚚 ۷٪ حمله + ۱۸٪ شانس غنیمت"},
}

def equipment_effects(data, uid):
    """قابلیت‌های تجهیزاتی که در موجودی کاربر فعال هستند؛ برای جلوگیری از اعداد افراطی، اثر هر نوع حداکثر ۵ بار لحاظ می‌شود."""
    eq = data.get("user_eq", {}).get(uid, {})
    out = {"attack_mult": 1.0, "def_mult": 1.0, "win_bonus": 0.0,
           "dmg_reduce": 0.0, "loot_bonus": 0.0, "coin_bonus": 0,
           "repair": 0}
    for name, ability in EQUIP_ABILITIES.items():
        n = min(5, max(0, int(eq.get(name, 0))))
        if not n: continue
        # درصدهای دفاع/حمله و شانس برد به صورت خطی و کنترل‌شده جمع می‌شوند.
        out["attack_mult"] *= 1 + (ability.get("attack_mult", 1.0)-1.0) * n
        out["def_mult"] *= 1 + (ability.get("def_mult", 1.0)-1.0) * n
        out["win_bonus"] += ability.get("win_bonus", 0.0) * n
        out["dmg_reduce"] += ability.get("dmg_reduce", 0.0) * n
        out["loot_bonus"] += ability.get("loot_bonus", 0.0) * n
        out["coin_bonus"] += ability.get("coin_bonus", 0) * n
        out["repair"] += ability.get("repair", 0) * n
    out["dmg_reduce"] = min(0.60, out["dmg_reduce"])
    out["win_bonus"] = min(0.25, out["win_bonus"])
    out["loot_bonus"] = min(0.75, out["loot_bonus"])
    return out

def equipment_special_for_attack(eq):
    return EQUIP_ABILITIES.get(eq, {}).get("desc", "⚔️ اثر ویژه ندارد")
...
async def do_attack(bot,cid,data,countries,uid,target,eq,amt):
    ...
    towner = tinfo["owner"]
    attacker_fx = equipment_effects(data, uid)
    defender_fx = equipment_effects(data, towner)
    tdef = int(defense_power(data,towner) * defender_fx["def_mult"])
    eq_power = EQUIP[eq][3]
    attack_power = int(amt*eq_power*atk_bonus*attacker_fx["attack_mult"])
    base_win = (attack_power/(attack_power+tdef)) if tdef>0 else 0.9
    win_ch = max(0.2, min(0.92, base_win + attacker_fx["win_bonus"]))
    won = random.random()<win_ch
    ...
    if won:
        ...
        # دفاع‌های ویژه کشور هدف قبل از ثبت خسارت اعمال می‌شوند.
        dmg = int(dmg * (1 - defender_fx["dmg_reduce"]))
        ...
        if target!="US" and countries[target]["damage_taken"]>=200000:
            if towner in data.get("users",{}):
                ...
                destroyed=True
        loot_text = ""
        if not destroyed and towner!="BOT_AI" and random.random()<min(0.95, 0.4 + attacker_fx["loot_bonus"]):
        ...
        addc(data,uid,100 + attacker_fx["coin_bonus"])
        # مرکز پزشکی کشور خودی بخشی از خسارت قبلی را ترمیم می‌کند.
        if attacker_fx["repair"] and attacker_code in countries:
            countries[attacker_code]["damage_taken"] = max(0, countries[attacker_code].get("damage_taken",0) - attacker_fx["repair"])
...
def get_main_menu():
    ...
    b.row(b.button(id="redeem_code",text="🎁 کد جایزه"))
    b.row(b.button(id="transfer_coins",text="🪙 انتقال سکه"),b.button(id="transfer_status",text="📊 سقف انتقال"))
...
def get_faction_menu():
    b=ChatKeypadBuilder(); row=[]
    for key,f in FACTIONS.items():
        row.append(b.button(id=f"faction_{key}",text=f"{f['emoji']} {f['name'][:10]}"))
        if len(row)==2:b.row(*row); row=[]
    if row:b.row(*row)
    b.row(b.button(id="faction_info",text="📊 اطلاعات"),b.button(id="back_to_menu",text="🏠 بازگشت")); return b.build(resize_keyboard=True,on_time_keyboard=True)
...
def get_admin_country_kb(countries,action_prefix,page=0):
    b=ChatKeypadBuilder(); lst=list(countries.items()); start=page*12; cur=lst[start:start+12]; row=[]
    for code,info in cur:
        owner="🟢" if info.get("owner") else "⚪"; row.append(b.button(id=f"{action_prefix}{code}",text=f"{info['flag']} {owner} {info['name'][:9]}"))
        if len(row)==3:b.row(*row); row=[]
    if row:b.row(*row)
    nav=[]
    if page>0:nav.append(b.button(id=f"ad_view_c_page_{page-1}",text="◀️ قبل"))
    nav.append(b.button(id="ad_back_to_admin",text="👑 ادمین"))
    if start+12<len(lst):nav.append(b.button(id=f"ad_view_c_page_{page+1}",text="بعد ▶️"))
    b.row(*nav); return b.build(resize_keyboard=True,on_time_keyboard=True)
...
def get_admin_panel(bs):
    b=ChatKeypadBuilder(); rows=[
        [("ad_stats","📊 آمار"),("ad_countries","🌍 کشورها")],
        [("ad_promo","📦 تبلیغ پک"),("ad_give_bonus","🎁 پاداش کاربر")],[("ad_users","👥 کاربران"),("ad_admins","👑 مدیران")],
        [("ad_broadcast","📣 همگانی"),("ad_reward_code","🎟️ ساخت کد")],[("ad_reward_list","📋 کدها"),("ad_reward_delete","🗑 حذف کد")],[("ad_add_coins","🪙 +کوین"),("ad_remove_coins","🪙 -کوین")],
        [("ad_add_pack","🎁 +پک"),("ad_add_eq","➕ +تجهیزات")],[("ad_remove_eq","➖ -تجهیزات"),("ad_un_manage","🌐 سازمان ملل")],
        [("ad_change_owner","🔄 تغییر مالک"),("ad_reset_country","♻️ ریست کشور")],[("ad_reset_damage","💥 ریست خسارت"),("ad_transfer_reset","🪙 ریست سقف انتقال")],
        [("ad_ban","🚫 مسدود"),("ad_unban","✅ رفع مسدود")],[("ad_warn","⚠️ اخطار"),("ad_unwarn","🧹 حذف اخطار")],
        [("ad_reset_user","👑 ریست کاربر"),("ad_give_country","🌍 اعطای کشور")],[("ad_user_search","🔎 جستجوی کاربر"),("ad_trade_stats","📈 تجارت")],
        [("ad_economy","💰 اقتصاد"),("ad_system","🛡 وضعیت سیستم")],[("ad_backup","💾 پشتیبان")]
    ]
    for r in rows:b.row(*(b.button(id=i,text=t) for i,t in r))
    b.row(b.button(id="ad_bot_off" if bs.get("online",True) else "ad_bot_on",text="🔴 خاموش کردن ربات" if bs.get("online",True) else "🟢 روشن کردن ربات"))
    b.row(b.button(id="ad_to_main_menu",text="🏠 منوی اصلی"),b.button(id="ad_close",text="🔒 خروج")); return b.build(resize_keyboard=True,on_time_keyboard=True)
...
def _hash_admin_password(password,salt=None):
    ...
    salt=salt or secrets.token_bytes(16); digest=hashlib.pbkdf2_hmac("sha256",password.encode(),salt,120000); return base64.b64encode(salt).decode()+":"+base64.b64encode(digest).decode()
def _verify_admin_password(password,stored):
    import hashlib,base64,hmac
    try:salt_b,digest_b=stored.split(":",1); salt=base64.b64decode(salt_b); digest=base64.b64decode(digest_b); test=hashlib.pbkdf2_hmac("sha256",password.encode(),salt,120000); return hmac.compare_digest(test,digest)
    except:return False
def is_full_admin(uid,data): return str(uid) in ADMINS
def admin_account_exists(uid,data): return str(uid) in data.get("admin_accounts",{})
def admin_can(uid,data,perm="full"):
    if str(uid) in ADMINS:return True
    acc=data.get("admin_accounts",{}).get(str(uid)); return bool(acc and (perm in acc.get("permissions",[]) or "full" in acc.get("permissions",[])))
...
async def handler(bot_instance:Robot,msg:Message):
    ...
    try:
        ...
        data=load_data()
        ensure_extra_data(data)
        is_admin = str(cid) in ADMINS or str(cid) in data.get("admin_accounts",{})
        ...
        if text=="/start":
            ...
            return
        if text=="/admin":
            user_states[cid]={"wait_pass":True}; await bot_instance.send_message(cid,"🔐 رمز پنل ادمین:"); return
        if user_states.get(cid,{}).get("wait_pass"):
            ok=(str(cid) in ADMINS and text==ADMIN_PASSWORD)
            acc=data.get("admin_accounts",{}).get(str(cid))
            if acc: ok=ok or _verify_admin_password(text,acc.get("password_hash",""))
            if ok:
                admin_session[cid]=True; user_states[cid]={}; await bot_instance.send_message(cid,"✅ ورود موفق به پنل مدیریت",chat_keypad=get_admin_panel(bs))
        ...
        if in_admin:
            ...
            if cb=="ad_countries":
                await bot_instance.send_message(cid,"🌍 مدیریت کشورها\nیک کشور را انتخاب کنید:",chat_keypad=get_admin_country_kb(countries,"ad_view_c_",0)); return
            if cb and cb.startswith("ad_view_c_page_"):
                page=int(cb.rsplit("_",1)[1]); await bot_instance.send_message(cid,"🌍 مدیریت کشورها:",chat_keypad=get_admin_country_kb(countries,"ad_view_c_",page)); return
            if cb and cb.startswith("ad_view_c_"):
                code=cb.replace("ad_view_c_",""); await bot_instance.send_message(cid,country_details(data,countries,code),chat_keypad=get_admin_country_detail_kb(code)); return
            if cb and cb.startswith("ad_country_reset_"):
                code=cb.replace("ad_country_reset_",""); await admin_reset_country(bot_instance,cid,data,countries,code); return
            if cb and cb.startswith("ad_country_owner_"):
                code=cb.replace("ad_country_owner_",""); user_states[cid]={"wait_change_owner_uid":True,"country_code":code}; await bot_instance.send_message(cid,"🆔 شناسه رئیس جدید:",chat_keypad=get_admin_back_cancel()); return
            if cb=="ad_reset_damage":
                for i in countries.values(): i["damage_taken"]=0
                save_countries(countries); await bot_instance.send_message(cid,"✅ خسارت تمام کشورها ریست شد.",chat_keypad=get_admin_back()); return
            if cb=="ad_transfer_reset":
                user_states[cid]={"reset_transfer":True}; await bot_instance.send_message(cid,"🆔 شناسه کاربر را بفرستید؛ برای همه بنویسید all",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("reset_transfer"):
                target=text.strip(); reset_transfer_limit(data,None if target.lower()=="all" else target); user_states[cid]={}; await bot_instance.send_message(cid,"✅ سقف انتقال ریست شد.",chat_keypad=get_admin_back()); return
            if cb=="ad_reward_code":
                user_states[cid]={"reward_code":True}; await bot_instance.send_message(cid,"🎟️ ساخت کد جایزه\nفرمت: CODE | COINS | USES | DAYS\nمثال: GNG500 | 500 | 10 | 7 | پهپاد:5,تانک:2",chat_keypad=get_admin_back_cancel()); return
            if cb=="ad_reward_list":
                codes=data.get("reward_codes",{}); lines=["🎟️ کدهای جایزه",""]
                for k,v in list(codes.items())[-30:]: lines.append(f"• `{k}` | 🪙 {fn(v.get('coins',0))} | {v.get('used',0)}/{v.get('uses',1)}")
                await bot_instance.send_message(cid,"\n".join(lines) if len(lines)>2 else "🎟️ هنوز کدی ساخته نشده.",chat_keypad=get_admin_back()); return
            if cb=="ad_reward_delete":
                user_states[cid]={"delete_reward":True}; await bot_instance.send_message(cid,"🎟️ کد برای حذف:",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("delete_reward"):
                code=text.strip().upper()
                if code in data.get("reward_codes",{}): data["reward_codes"].pop(code); data.get("reward_code_claims",{}).pop(code,None); save_data(data); await bot_instance.send_message(cid,"✅ کد حذف شد.")
                else: await bot_instance.send_message(cid,"❌ کد پیدا نشد.")
                user_states[cid]={}; return
            if user_states.get(cid,{}).get("reward_code"):
                try:
                    parts=[x.strip() for x in text.split("|")]; code=parts[0]; coins=int(parts[1]); uses=int(parts[2]); days=int(parts[3]) if len(parts)>3 else 0
                    reward_eq={}
                    if len(parts)>4 and parts[4]:
                        for chunk in parts[4].split(","):
                            en,ea=chunk.split(":",1); ea=int(ea); 
                            if en in EQUIP and ea>0: reward_eq[en]=ea
                    ok,res=create_reward_code(data,code,coins,reward_eq,uses,days); await bot_instance.send_message(cid,res if not ok else f"✅ کد ساخته شد: `{res}`\n🪙 {fn(coins)} | 🔢 {uses} استفاده\n🧰 {', '.join(f'{k}×{v}' for k,v in reward_eq.items()) or 'بدون تجهیزات'}")
                except Exception: await bot_instance.send_message(cid,"❌ فرمت اشتباه است.")
                user_states[cid]={}; return
            if cb=="ad_admins":
                lines=["👑 مدیران بازی","",f"⭐ مدیر اصلی: {len(ADMINS)}"]
                for au,ai in data.get("admin_accounts",{}).items(): lines.append(f"• `{au}` | {ai.get('name','مدیر')} | {ai.get('role','manager')}")
                b=ChatKeypadBuilder(); b.row(b.button(id="ad_reset_admin_pass",text="🔑 رمز جدید"),b.button(id="ad_back_to_admin",text="👑 ادمین"))
                await bot_instance.send_message(cid,"\n".join(lines),chat_keypad=b.build(resize_keyboard=True,on_time_keyboard=True)); return
            if cb=="ad_reset_admin_pass":
                if not is_full_admin(cid,data): return await bot_instance.send_message(cid,"🛡️ فقط مدیر اصلی می‌تواند رمز مدیر را تغییر دهد.")
                user_states[cid]={"reset_admin_pass":True}; await bot_instance.send_message(cid,"🆔 شناسه مدیر:",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("reset_admin_pass"):
                target=text.strip()
                if target not in data.get("admin_accounts",{}): await bot_instance.send_message(cid,"❌ مدیر پیدا نشد.")
                else:
                    pwd=new_admin_password(); data["admin_accounts"][target]["password_hash"]=_hash_admin_password(pwd); save_data(data); await bot_instance.send_message(cid,f"✅ رمز جدید ساخته شد.\n🆔 `{target}`\n🔐 `{pwd}`")
                user_states[cid]={}; return
            if cb=="ad_users":
                txt="👥 کاربران:\n"
            ...
            if cb=="ad_promo":
                featured=max(PACKS.items(),key=lambda x:x[1][0])[0] if PACKS else ""
                await bot_instance.send_message(cid,f"📦 پیام تبلیغاتی آماده\n\n⭐ پیشنهاد فروشگاه\n🎁 {featured}\n💰 قیمت: {fn(PACKS[featured][0])} تومان\n✨ تجهیزات بیشتر = پیشرفت سریع‌تر در بازی\n📩 خرید: {ADMIN_USERNAME}",chat_keypad=get_admin_back()); return
            if cb=="ad_give_bonus":
                user_states[cid]={"bonus_user":True}; await bot_instance.send_message(cid,"🎁 فرمت: شناسه | مقدار کوین",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("bonus_user"):
                try:
                    target,amt=text.split("|",1); target=target.strip(); amt=int(amt.strip())
                    if target not in data["users"] or amt<=0: raise ValueError
                    addc(data,target,amt); await bot_instance.send_message(cid,f"✅ {fn(amt)} کوین به کاربر داده شد.")
                except: await bot_instance.send_message(cid,"❌ فرمت اشتباه است.")
                user_states[cid]={}; return
            if cb=="ad_broadcast":
            ...
            if user_states.get(cid,{}).get("add_eq"):
                try:
                    ...
                    target=parts[0]
                    count=int(parts[-1])
                    eq_name=" ".join(parts[1:-1]).strip()
                    if count<=0: await bot_instance.send_message(cid,"❌ تعداد باید مثبت باشد")
                    elif target not in data["users"]: await bot_instance.send_message(cid,"❌ کاربر یافت نشد!")
                    elif eq_name not in EQUIP: await bot_instance.send_message(cid,"❌ تجهیزات نامعتبر! نام تجهیز را دقیق وارد کنید.")
                    else: addeq(data,target,eq_name,count); await bot_instance.send_message(cid,f"✅ {count} {eq_name} اضافه شد.")
                except: await bot_instance.send_message(cid,"❌ فرمت اشتباه؛ نمونه: 123456789 هوش مصنوعی فرماندهی 1")
                user_states[cid]={}; return
            if cb=="ad_remove_eq":
        ...
        # user menu
        if cb=="transfer_coins":
            user_states[cid]={"transfer_target":True}; await bot_instance.send_message(cid,"🪙 شناسه گیرنده را وارد کنید:",chat_keypad=get_cancel()); return
        if user_states.get(cid,{}).get("transfer_target"):
            target=text.strip()
            if target not in data.get("users",{}): await bot_instance.send_message(cid,"❌ کاربر پیدا نشد.")
            else: user_states[cid]={"transfer_amount":True,"target":target}; await bot_instance.send_message(cid,"💸 مقدار سکه را وارد کنید.\n📌 سقف انتقال روزانه: ۵,۰۰۰")
            return
        if user_states.get(cid,{}).get("transfer_amount"):
            target=user_states[cid]["target"]; ok,res=transfer_coins_safe(data,uid,target,text.strip()); await bot_instance.send_message(cid,(f"✅ انتقال انجام شد.\n🧾 تراکنش: `{res}`\n📊 مصرف امروز: {fn(daily_transfer_used(data,uid))}/۵,۰۰۰" if ok else res),chat_keypad=get_main_menu())
            if ok:
                try: await bot_instance.send_message(target,f"🪙 {fn(int(text))} سکه به حساب شما منتقل شد.")
                except: pass
            user_states[cid]={}; return
        if cb=="transfer_status":
            used=daily_transfer_used(data,uid); await bot_instance.send_message(cid,f"📊 سقف انتقال روزانه\n🪙 مصرف‌شده: {fn(used)}\n🪙 باقی‌مانده: {fn(MAX_DAILY_TRANSFER-used)}",chat_keypad=get_main_menu()); return
        if cb=="redeem_code":
            user_states[cid]={"redeem_code":True}; await bot_instance.send_message(cid,"🎁 کد جایزه را وارد کنید:",chat_keypad=get_cancel()); return
        if user_states.get(cid,{}).get("redeem_code"):
            ok,res=redeem_reward_code(data,uid,text); await bot_instance.send_message(cid,res,chat_keypad=get_main_menu()); user_states[cid]={}; return
        if cb=="trade_menu": await bot_instance.send_message(cid,"📈 مرکز تجارت جهانی",chat_keypad=get_trade_menu()); return
        if cb=="trade_market":
            market_tick(data); lines=["📈 بازار جهانی","","قیمت لحظه‌ای:"]+[f"{m['emoji']} {k}: 🪙 {fn(data['market'][k])}" for k,m in MARKET_ITEMS.items()]; await bot_instance.send_message(cid,"\n".join(lines),chat_keypad=get_trade_menu()); return
        if cb=="trade_inventory":
            inv=data["users"][uid].get("trade_inventory",{}); lines=["📦 انبار تجارت",""]+[f"• {MARKET_ITEMS[k]['emoji']} {k}: {fn(v)}" for k,v in inv.items() if k in MARKET_ITEMS and v>0]; await bot_instance.send_message(cid,"\n".join(lines) if len(lines)>2 else "📦 انبار خالی است.",chat_keypad=get_trade_menu()); return
        if cb in ("trade_buy","trade_sell"):
            user_states[cid]={"trade":"buy" if cb=="trade_buy" else "sell"}; await bot_instance.send_message(cid,"فرمت: نام کالا | تعداد\nمثال: طلا | 5",chat_keypad=get_cancel()); return
        if user_states.get(cid,{}).get("trade"):
            try:item,qty=[x.strip() for x in text.split("|",1)]; ok,res=trade_item(data,uid,item,int(qty),user_states[cid]["trade"]); await bot_instance.send_message(cid,res,chat_keypad=get_trade_menu())
            except: await bot_instance.send_message(cid,"❌ فرمت اشتباه است.")
            user_states[cid]={}; return
        if cb=="rules":
        ...
        if cb=="alliance_menu":
            aname,_=get_al(adata,uid)
            await bot_instance.send_message(
                cid,
                "🤝 مدیریت اتحاد\n\n" + (f"🏰 اتحاد شما: {aname}" if aname else "❌ شما عضو هیچ اتحادی نیستید.\nاز همین‌جا می‌توانید اتحاد بسازید یا به یک اتحاد موجود بپیوندید."),
                chat_keypad=get_alliance_menu(bool(aname), is_leader(adata,uid))
            )
            return
        if cb=="battle_report":
        ...
        if cb=="alliance_list" or (cb and cb.startswith("alliance_page_")):
            alliances=list(adata.get("alliances",{}).items())
            if not alliances:
                return await bot_instance.send_message(cid,"📋 هیچ اتحادی وجود ندارد.",chat_keypad=get_alliance_menu(False,False))
            page=0
            if cb and cb.startswith("alliance_page_"):
                try: page=max(0,int(cb.rsplit("_",1)[1]))
                except: page=0
            per_page=6
            start=page*per_page
            current=alliances[start:start+per_page]
            txt=f"📋 اتحادهای موجود (صفحه {page+1}/{(len(alliances)+per_page-1)//per_page})\n\n"
            builder=ChatKeypadBuilder()
            my_name,_=get_al(adata,uid)
            for i,(a_name,a_info) in enumerate(current,start+1):
                leader_name=data["users"].get(a_info.get("leader"),{}).get("username","نامشخص")
                safe_name=str(a_name)[:28]
                txt+=f"{i}. 🤝 {safe_name}\n   👑 {str(leader_name)[:25]} | 👥 {len(a_info.get('members',[]))}\n"
                if not my_name:
                    # Keep callback IDs short and independent of the alliance name.
                    builder.row(builder.button(id=f"join_alliance_{start+i-(start+1)}",text=f"🤝 پیوستن به {safe_name}"))
            nav=[]
            if page>0: nav.append(builder.button(id=f"alliance_page_{page-1}",text="◀️ قبل"))
            nav.append(builder.button(id="alliance_menu",text="🔙 بازگشت"))
            if start+per_page<len(alliances): nav.append(builder.button(id=f"alliance_page_{page+1}",text="بعد ▶️"))
            builder.row(*nav)
            # Store the current page mapping so long/special alliance names never enter callback IDs.
            data.setdefault("_alliance_pages",{})[str(uid)]={str(j):a_name for j,(a_name,_) in enumerate(current)}
            save_data(data)
            await bot_instance.send_message(cid,txt[:3500],chat_keypad=builder.build(resize_keyboard=True,on_time_keyboard=True)); return
        if cb and cb.startswith("join_alliance_"):
            idx=cb.replace("join_alliance_","")
            mapping=data.get("_alliance_pages",{}).get(str(uid),{})
            a_name=mapping.get(str(idx))
            if not a_name:
                return await bot_instance.send_message(cid,"❌ این صفحه منقضی شده؛ دوباره لیست اتحادها را باز کنید.",chat_keypad=get_alliance_menu(False,False))
            if a_name not in adata.get("alliances",{}): return await bot_instance.send_message(cid,"❌ اتحاد یافت نشد.")
            ...
            await bot_instance.send_message(cid,f"✅ شما به اتحاد {a_name[:50]} پیوستید!",chat_keypad=get_main_menu())
        ...
        if cb=="alliance_info":
            ...
            for m in info.get("members",[])[:25]:
                member_name=str(data["users"].get(m,{}).get("username",m[:10]))[:22]
                member_lines.append(f"{'👑' if m==info['leader'] else '👤'} {member_name}")
            if len(info.get("members",[]))>25:
                member_lines.append(f"… و {len(info['members'])-25} عضو دیگر")
            members="\n".join(member_lines) or "—"
            out=f"🔍 اتحاد {str(name)[:40]}\n👑 رهبر: {str(leader_name)[:30]}\n👥 اعضا ({len(info.get('members',[]))}):\n{members}"
            await bot_instance.send_message(cid,out[:3500],chat_keypad=get_alliance_menu(True, info.get("leader")==uid)); return
        if cb=="alliance_chat":
        ...
        for pn in PACKS:
            if cb==f"shop_{pn}":
                items=PACKS[pn][3] if len(PACKS[pn])>3 else {}
                preview=" | ".join(f"{k}×{v}" for k,v in list(items.items())[:5])
                await bot_instance.send_message(cid,f"📦 {pn}\n💰 {fn(PACKS[pn][0])} تومان\n✨ تقویت سریع‌تر ارتش و پیشرفت در بازی\n🧰 {preview}\n\nبرای خرید و دریافت راهنمای پرداخت به {ADMIN_USERNAME} پیام دهید."); return
        if cb=="send_message":
        ...
        if cb=="top_owners":
            ...
            for c,i in countries.items():
                if i.get("owner"):
                    ...
                    leader_flag=(own==un.get("leader")); own_name="🤖 ربات" if own=="BOT_AI" else data["users"].get(own,{}).get("username",own[:15])
                    tops.append({"flag":i["flag"],"name":i["name"],"power":pwr+(10000 if leader_flag else 0),"damage":i.get("damage_taken",0),"owner":own_name,"uid":own,"is_leader":leader_flag})
