    ...
    try:
        ...
        if cb=="alliance_list":
            ...
            for i,(a_name,a_info) in enumerate(alliances.items(),1):
                ...
                txt+=f"{i}. 🤝 {a_name}\n   👑 رهبر: {leader_name} | 👥 اعضا: {len(a_info['members'])}\n"
                if not my_name:
                    builder.row(builder.button(id=f"join_alliance_{a_name}",text=f"🤝 پیوستن به {a_name}"))
            builder.row(builder.button(id="alliance_menu",text="🔙 بازگشت به اتحادها"))
            await bot_instance.send_message(cid,txt,chat_keypad=builder.build(resize_keyboard=True,on_time_keyboard=True)); return
