...
def get_main_menu():
    ...
    b.row(b.button(id="trade_menu",text="📈 تجارت"),b.button(id="redeem_code",text="🎁 کد جایزه"))
...
def get_admin_panel(bs):
    b=ChatKeypadBuilder(); rows=[
        ...
        [("ad_add_admin","➕ افزودن مدیر"),("ad_remove_admin","❌ اخراج مدیر")],
...
# ---------- admin accounts ----------
# رمزهای مدیران عمداً ساده و به‌صورت متن ساده نگهداری می‌شوند.
# توجه: این روش برای پروژه‌های شخصی مناسب‌تر است و برای سرویس عمومی امن نیست.
def _plain_admin_password(password):
    return str(password).strip()
def _verify_admin_password(password,stored):
    return str(password).strip() == str(stored).strip()
...
def migrate_admin_passwords(data):
    # مدیرانی که از نسخه قبلی آمده‌اند رمز هش‌شده داشتند؛
    # چون رمز قبلی قابل بازیابی نیست، یک رمز ساده جدید دریافت می‌کنند.
    changed=False
    for uid,acc in data.get("admin_accounts",{}).items():
        if "password" not in acc:
            acc.pop("password",None)
            acc["password"]="ADM1234"
            changed=True
...
async def handler(bot_instance:Robot,msg:Message):
    ...
    try:
        ...
        if user_states.get(cid,{}).get("creating_alliance"):
            ...
            await bot_instance.send_message(cid,f"✅ اتحاد {new_name} ایجاد شد!\n\n👑 شما رهبر اتحاد هستید.",chat_keypad=get_alliance_menu(True,True))
            user_states[cid]={}; return
        ...
        if user_states.get(cid,{}).get("wait_pass"):
            ...
            if acc: ok=ok or _verify_admin_password(text,acc.get("password",""))
            if ok:
        ...
        if in_admin:
            ...
            if cb=="ad_admins":
                ...
                b=ChatKeypadBuilder(); b.row(b.button(id="ad_add_admin",text="➕ افزودن مدیر"),b.button(id="ad_remove_admin",text="❌ اخراج مدیر")); b.row(b.button(id="ad_reset_admin_pass",text="🔑 رمز جدید"),b.button(id="ad_back_to_admin",text="👑 ادمین"))
            ...
            if user_states.get(cid,{}).get("reset_admin_pass"):
                ...
                if target not in data.get("admin_accounts",{}): await bot_instance.send_message(cid,"❌ مدیر پیدا نشد.")
                    ...
                    pwd=new_admin_password(); data["admin_accounts"][target].pop("password",None); data["admin_accounts"][target]["password"]=pwd; save_data(data); await bot_instance.send_message(cid,f"✅ رمز جدید ساخته شد.\n🆔 `{target}`\n🔐 `{pwd}`")
                user_states[cid]={}; return
            if cb=="ad_add_admin":
                if not is_full_admin(cid,data): return await bot_instance.send_message(cid,"🛡️ فقط مدیر اصلی می‌تواند مدیر اضافه کند.")
                user_states[cid]={"add_admin":True}; await bot_instance.send_message(cid,"فرمت: شناسه | نام | نقش\nنقش‌ها: manager / moderator"); return
            if cb=="ad_remove_admin":
                if not is_full_admin(cid,data): return await bot_instance.send_message(cid,"🛡️ فقط مدیر اصلی.")
                user_states[cid]={"remove_admin":True}; await bot_instance.send_message(cid,"🆔 شناسه مدیر برای اخراج:",chat_keypad=get_admin_back_cancel()); return
            if user_states.get(cid,{}).get("add_admin"):
                try:
                    parts=[x.strip() for x in text.split("|")]; target=parts[0]; name=parts[1] if len(parts)>1 else "مدیر"; role=parts[2] if len(parts)>2 else "manager"
                    if target not in data["users"]: await bot_instance.send_message(cid,"❌ کاربر باید حداقل یک‌بار ربات را استارت کرده باشد.")
                    elif target in ADMINS or target in data["admin_accounts"]: await bot_instance.send_message(cid,"⚠️ این شناسه از قبل مدیر است.")
                    else:
                        pwd=new_admin_password(); data["admin_accounts"][target]={"name":name,"role":role,"password":pwd,"permissions":["full" if role=="manager" else "moderation"],"created":datetime.now().isoformat()}; save_data(data); await bot_instance.send_message(cid,f"✅ مدیر اضافه شد.\n🆔 `{target}`\n👤 {name}\n🔐 رمز: `{pwd}`")
                except Exception: await bot_instance.send_message(cid,"❌ فرمت اشتباه است.")
                user_states[cid]={}; return
            if user_states.get(cid,{}).get("remove_admin"):
                target=text.strip()
                if target in data.get("admin_accounts",{}): data["admin_accounts"].pop(target); save_data(data); admin_session.pop(target,None); await bot_instance.send_message(cid,"✅ مدیر اخراج شد.")
                else: await bot_instance.send_message(cid,"❌ مدیر پیدا نشد.")
                user_states[cid]={}; return
            if cb=="ad_users":
                txt="👥 کاربران:\n"
        ...
        if cb=="alliance_menu":
            ...
            is_member=bool(aname)
            is_leader=bool(aname and adata.get("alliances",{}).get(aname,{}).get("leader")==uid)
            await bot_instance.send_message(
                cid,
                mh("🤝 مرکز اتحادها")+"\n\n"+(f"🏷️ اتحاد فعلی: {aname}" if aname else "❌ شما عضو هیچ اتحادی نیستید\n\nاز همین‌جا می‌توانید اتحاد بسازید یا اتحادهای موجود را ببینید."),
                chat_keypad=get_alliance_menu(is_member,is_leader)
            )
            return
        if cb=="battle_report":
